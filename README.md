...
|---|---|---|
| **Perception** | `MAX_DAILY_MOVE` plausibility bound in `update_navs()`, rejecting implausible or malformed NAV values before they reach holdings. | `control_plane.test_acp1_adapter_publishes_rejections` / `_silent_when_nothing_rejected` (2/2, passing) exercise the ACP-1 signal adapter wrapping `update_navs()`'s `rejected`/`failures` return values. `Tests/test_perception.py` cannot currently run: no `conftest.py` defines the `tool_registry` fixture it requires, and its call `tool_registry.get("update_navs").func(...)` targets a `func` attribute that doesn't exist on `CTool` (the field is `mTool_func`). |
